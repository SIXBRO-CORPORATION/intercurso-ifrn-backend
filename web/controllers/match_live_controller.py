...
async def stream_match_events(
    ...
    """UC016: canal de tempo real de uma partida específica, via SSE.

    Autenticação por ticket de curta duração (seção 4, cuidado #1) �?" não por
    Bearer token na query string. O front deve:
    1. Chamar `POST /api/realtime/ticket` (autenticado normalmente) pedindo
       um ticket para `{"channel_type": "match", "channel_id": match_id}`.
    2. Abrir `new EventSource('/api/match/{match_id}/live?ticket=...')` com
       o ticket recebido.
    3. Se a conexão cair, abrir a nova conexão SSE e então reconciliar o estado
       via `GET /api/match/{match_id}`.
    """
