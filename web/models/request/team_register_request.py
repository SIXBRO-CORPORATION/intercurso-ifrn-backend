...
class TeamRegisterRequest(BaseModel):
    ...
    @classmethod
    def validate_members_unique(cls, v: List[TeamMemberRequest]) -> List[TeamMemberRequest]:
        matriculas = [member.matricula for member in v]
        if len(matriculas) != len(set(matriculas)):
            raise ValueError('Não é permitido membros com matrículas duplicadas no mesmo time')

        cpfs = [member.cpf for member in v]
        if len(cpfs) != len(set(cpfs)):
            raise ValueError('Não é permitido membros com CPFs duplicados no mesmo time')
