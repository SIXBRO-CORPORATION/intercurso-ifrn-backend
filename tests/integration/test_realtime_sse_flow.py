"""Teste de integração do fluxo de ticket e streaming SSE:

"publicar um evento via Command de business/match/ e confirmar que uma
conexão SSE assinada no canal correspondente recebe a mensagem no formato
esperado".

Aqui simplificamos o "publicar via Command" para "publicar via broadcaster
diretamente", já que o Command real depende de banco de dados; o que este
teste garante é a ponta a ponta do transporte: ticket -> assinatura SSE ->
publish -> mensagem recebida no formato `event:`/`data:` esperado.
"""
...
class _ServerThread(threading.Thread):
    """Sobe a app real num servidor HTTP de verdade numa porta local.

    Necessário porque `httpx.ASGITransport` não faz streaming incremental de
    verdade: ele roda a aplicação ASGI até o fim antes de devolver a
    resposta, então nunca entrega um chunk de SSE antes do generator
    terminar (o que, para um canal que só termina quando o cliente
    desconecta, nunca acontece). Um servidor HTTP real resolve isso.
    """
