...
def make_context(
    ...
    context.put_property(
        "created_by",
        created_by if created_by is not None else uuid4(),
    )
...
class TestCreateModalityAdapter:
    async def test_creates_modality_and_configuration_successfully(self):
        ...
        modality_configuration_repository.save.side_effect = (
        ...
        assert audit_call_kwargs["actor_id"] == context.get_property(
            "created_by",
            UUID,
        )
    ...
    async def test_creates_volleyball_configuration_when_score_type_is_sets(
        self,
    ):
        (
            adapter,
        ...
        volleyball_repository.save.side_effect = (
            lambda config: config
        )
        ...
        volleyball_repository.save.assert_awaited_once()

        saved_volleyball_arg = (
            volleyball_repository.save.await_args.args[0]
        )

        assert (
            saved_volleyball_arg.modality_configuration_id
