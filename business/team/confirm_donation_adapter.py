...
class ConfirmDonationAdapter(ConfirmDonationPort):
    ...
    async def execute(self, context: Context) -> TeamMember:
        ...
        member = await self.team_member_repository.find_member_by_matricula_and_team_id(matricula, team_id)

        if member is None:
            raise BusinessException(f"Membro com matrícula {matricula} não encontrado neste time")
