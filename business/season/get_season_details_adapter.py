...
class GetSeasonDetailsAdapter(GetSeasonDetailsPort):
    ...
    async def execute(self, context: Context) -> Season:
        ...
        teams_summary = await self.team_repository.count_teams_summary_by_season(
            season_id
        )

        context.put_property("season_modalities", season_modalities)
        context.put_property(
            "total_teams_created", teams_summary["total_teams_created"]
        )
        context.put_property(
            "total_teams_submitted", teams_summary["total_teams_submitted"]
        )
        context.put_property(
            "total_teams_approved", teams_summary["total_teams_approved"]
        )
        context.put_property(
            "available_actions",
