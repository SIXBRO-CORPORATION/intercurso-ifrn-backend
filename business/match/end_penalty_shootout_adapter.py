...
from business.match._finish_shared import ensure_penalty_shootout_active, finalize_match
...
class EndPenaltyShootoutAdapter(EndPenaltyShootoutPort):
    ...
    async def execute(self, context: Context) -> Match:
        ...
        team1_penalties = match.team1_penalty_score or 0
        team2_penalties = match.team2_penalty_score or 0

        if team1_penalties == team2_penalties:
            raise BusinessException(
                "Pênaltis ainda empatados "
                f"({team1_penalties}x{team2_penalties}). Continue registrando "
                "cobranças (morte súbita) antes de encerrar a disputa."
            )

        winner_id = match.team1_id if team1_penalties > team2_penalties else match.team2_id
        penalty_result = {
        ...
        match.penalty_shootout_active = False
