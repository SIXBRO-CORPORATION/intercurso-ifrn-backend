...
class DeleteEventAdapter(DeleteEventPort):
    ...
    async def execute(self, context: Context) -> Match:
        ...
        event_id = context.get_property("event_id", UUID)

        match = await validate_match_correctable(
            self.match_repository, match_id, monitor_id
        )

        target_event = await get_correctable_event(
            self.match_event_repository, match_id, event_id
        )
