...
>
> **Atualizado em 2026-08-17** após a conclusão do UC014 (Registrar Evento) dentro da Fase 5. O diagnóstico e a tabela abaixo refletem o estado real do código nesta data, não mais o estado descrito nas versões anteriores deste plano.
...
- **Business (casos de uso reais):** implementado para Autenticação, Usuário (criar/buscar/atualizar perfil), **Temporada (UC001, UC002 e UC003 completos)**, **Modalidade (UC004 completo)**, **Equipe (UC005 a UC010 completos)**, **Chaveamento (UC011 e UC012 completos)** e, dentro de Partida, **UC013 (Iniciar Partida) e UC014 (Registrar Evento) completos**: gol/ponto, cartão amarelo/vermelho com expulsão automática no 2º amarelo ou vermelho direto, pausar/retomar cronômetro, encerrar/iniciar período e encerrar set (vôlei, com sugestão automática de fim de partida ao atingir os sets necessários). UC015 e UC017 seguem sem lógica de negócio.
- **Web (controllers/rotas):** `auth_controller`, `user_controller`, `season_controller` e `modality_controller` funcionais e registrados em `main.py`. `team_controller` completo para o ciclo de vida do time: criar, entrar via convite, selecionar capitão, remover membro, sair do time, submeter para aprovação, aprovar e confirmar doação. `bracket_controller` completo para criar chaveamento (com preview de configuração), re-sortear, editar e deletar partidas. `match_controller` cobre início de partida e todos os sub-fluxos do UC014 (`POST /api/match/{match_id}/goal`, `/card`, `/clock/pause`, `/clock/resume`, `/period/end`, `/period/start`, `/set/end`), todos exigindo ator Monitor e devolvendo a mesma resposta rica (`MatchManagementResponse`) usada pelo `start`.
- **Testes:** `tests/unit/business` cobre os adapters de `season`, `modality`, `team` (parcialmente), `bracket` (motor de sorteio + os 4 adapters) e, agora, `match` — 33 testes cobrindo `start_match`, `register_goal`, `register_card` (incluindo 2º amarelo e vermelho direto), cronômetro (pausar/retomar) e período/set. Ainda não há testes de `tests/integration` nem `tests/e2e`.
...
- **Auditoria:** existe infraestrutura própria (`domain/audit/audit_log.py`, `core/business/audit/audit_logger.py`, tabela `logs`) e já é usada em `season`, `team`, `bracket` e no `start_match_adapter` (UC013). **Os sete adapters do UC014 (gol, cartão, pausar/retomar cronômetro, período, set) não chamam o `AuditLogger`** e não há valores no enum `AuditAction` para esses eventos (só existe `MATCH_STARTED`) — RN41-42 do UC014 ("todos os eventos devem ser registrados para auditoria") está, portanto, incompleta.
...
### Fase 5 — Gestão de Partidas (UC013 ✅ CONCLUÍDA · UC014 ✅ CONCLUÍDA · UC015, UC017 pendentes)
...
- **Pendente nesta fase:** UC015 (Finalizar Partida) e UC017 (Corrigir Evento) — ficam para as próximas rodadas, um PR por caso de uso, dado o volume de regras de negócio de cada um (~20KB de especificação cada). UC015 é quem vai efetivamente precisar de uma estratégia de avanço automático no chaveamento (`next_match_id` ou equivalente, observação já registrada ao final da Fase 4 — ainda não resolvida; nenhuma ocorrência de `next_match_id` no código); UC013 e UC014 não mexeram nisso por não precisarem.
- Também não implementado até aqui (fora do escopo de UC013/UC014): endpoints de consulta de partida (`GET /api/match/{match_id}`, listagens por temporada/time) — mesmo padrão de débito técnico já assumido para os `GET`s de chaveamento na Fase 4.
...
- Esta fase depende funcionalmente da Fase 5 estar concluída (os eventos precisam existir antes de serem transmitidos) — UC013/UC014 já geram eventos; falta UC015/UC017 para o conjunto completo do Bloco de Dados 4 do UC016.
...
Fase 3 (Equipes, UC006-008)  →  ✅ concluída
        ↓
Fase 4 (Chaveamento, UC011-012)  →  ✅ concluída  →  Fase 5 (Partidas, UC013 ✅ / UC014 ✅ / UC015-017 pendentes)  →  Fase 6 (Tempo real)
...
7. Fechar o débito técnico introduzido pelo UC014 antes de avançar para o UC015: adicionar chamadas ao `AuditLogger` (e os valores correspondentes em `AuditAction`) nos sete adapters de `business/match/` que hoje não registram auditoria (gol, cartão, pausar/retomar cronômetro, período, set) — ver observação na Fase 5. Também vale atualizar o `Status` do [ADR 0002](../adr/ADR002_NormalizacaoModalidade.md) para refletir que a normalização já foi implementada.
8. Priorizar UC015 (Finalizar Partida) como próximo passo da Fase 5 — é o caso de uso que efetivamente precisa da estratégia de avanço automático no chaveamento (`next_match_id` ou equivalente), ainda em aberto desde a observação registrada ao final da Fase 4.
9. Depois de UC015, fechar a Fase 5 com UC017 (Corrigir Evento) e então seguir para Fases 6–7 na ordem já prevista, mantendo a Fase 8 (testes/CI/docs) em paralelo contínuo — incluindo a dívida de testes unitários de UC005 a UC010 registrada na Fase 3 e os `GET`s de chaveamento e de partida registrados como débito técnico nas Fases 4 e 5.