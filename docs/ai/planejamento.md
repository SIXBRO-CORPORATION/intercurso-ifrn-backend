...
- **Business (casos de uso reais):** implementado para Autenticação, Usuário (criar/buscar/atualizar perfil), **Temporada (UC001, UC002 e UC003 completos)**, **Modalidade (UC004 completo)**, **Equipe (UC005 a UC010 completos)**, **Chaveamento (UC011 e UC012 completos)** e, dentro de Partida, **UC013 (Iniciar Partida), UC014 (Registrar Evento), UC015 (Finalizar Partida) e UC017 (Corrigir Evento) completos**: gol/ponto, cartão amarelo/vermelho com expulsão automática no 2º amarelo ou vermelho direto, pausar/retomar cronômetro, encerrar/iniciar período, encerrar set (vôlei, com sugestão automática de fim de partida ao atingir os sets necessários), finalizar partida com avanço automático no chaveamento (vencedor → próxima fase, perdedor de semifinal → 3º lugar), disputa de pênaltis completa (iniciar/registrar cobrança/encerrar) para o desempate em mata-mata, e agora desfazer/deletar eventos corrigíveis (inclusive em partida já `FINISHED`, com destaque especial de auditoria e alerta crítico se o placar mudar).
- **Web (controllers/rotas):** `auth_controller`, `user_controller`, `season_controller` e `modality_controller` funcionais e registrados em `main.py`. `team_controller` completo para o ciclo de vida do time: criar, entrar via convite, selecionar capitão, remover membro, sair do time, submeter para aprovação, aprovar e confirmar doação. `bracket_controller` completo para criar chaveamento (com preview de configuração), re-sortear, editar e deletar partidas. `match_controller` cobre início de partida, todos os sub-fluxos do UC014 (`POST /api/match/{match_id}/goal`, `/card`, `/clock/pause`, `/clock/resume`, `/period/end`, `/period/start`, `/set/end`), o UC015 (`POST /api/match/{match_id}/finish`, `/penalty-shootout/start`, `/penalty-shootout/kick`, `/penalty-shootout/end`) e agora o UC017 (`POST /api/match/{match_id}/event/undo`, `DELETE /api/match/{match_id}/event/{event_id}`), todos exigindo ator Monitor e devolvendo a mesma resposta rica (`MatchManagementResponse`) usada pelo `start`.
- **Testes:** `tests/unit/business` cobre os adapters de `season`, `modality`, `team` (parcialmente), `bracket` (motor de sorteio + os 4 adapters, agora também o encadeamento de `next_match_id`) e `match` — 246 testes no total da suíte, sendo 67 no módulo `match` (`start_match`, UC014 completo, `finish_match`, os 3 adapters de pênaltis do UC015 e os 22 de UC017 em `test_correction_adapters.py`). Ainda não há testes de `tests/integration` nem `tests/e2e`.
...
### Fase 5 — Gestão de Partidas (UC013 ✅ CONCLUÍDA · UC014 ✅ CONCLUÍDA · UC015 ✅ CONCLUÍDA · UC017 ✅ CONCLUÍDA) ✅ FASE CONCLUÍDA
...
- **UC017 — Corrigir Evento — CONCLUÍDO:**
  - **Ponto de partida da rodada:** um handoff prévio (`docs/spec/UC017_GestãoDePartidas_CorrigirEvento.md` + investigação de código) identificou uma contradição entre o [ADR 0002](../adr/ADR002_NormalizacaoModalidade.md) (que previa ajustar/remover a linha de `match_set` na correção) e a trigger de banco criada junto com `match_sets` (`prevent_match_sets_mutation()`), que bloqueava qualquer `UPDATE`/`DELETE` sem distinguir sobrescrita acidental de correção legítima auditada.
  - **Decisão de imutabilidade (Opção A — soft delete, não remoção da trigger):** em vez de descartar a proteção do ADR 0002, a trigger foi reescrita para permitir **apenas** a transição `deleted_at` NULL → preenchido (nenhuma outra coluna pode mudar num `UPDATE`); `DELETE`/`TRUNCATE` continuam bloqueados sempre. A `UniqueConstraint(match_id, set_number)` virou um índice único parcial (`WHERE deleted_at IS NULL`), permitindo inserir uma nova linha correta no lugar da antiga soft-deletada.
  - **Descoberta que reduziu o escopo da migration:** `match_events` e `match_sets` já herdavam `deleted_at`/`active` de `AbstractEntity` desde as migrations v1/v6 — não foi necessário adicionar nenhuma coluna nova; só a regra de mutabilidade de `match_sets` mudou. `MatchEventRepositoryPort` já tinha `soft_delete_event`/`find_active_by_match`/`find_last_event_by_match` prontos e não usados por nenhum outro UC — claramente scaffolding deixado para esta rodada. `MatchSetRepositoryPort` ganhou o método análogo `soft_delete_set`.
  - **Decisões de escopo tomadas no início da rodada:**
    1. "Último evento desta sessão/dispositivo" (Fluxo Principal) foi simplificado para **"último evento não estrutural da partida"**, já que o domínio não rastreia sessão/dispositivo hoje e, na prática, só um monitor opera uma partida por vez.
    2. Correção em partida `FINISHED`: **qualquer Monitor do sistema** pode corrigir (cenário de revisão pós-jogo), sem exigir permissão extra de Admin/Coordenador.
    3. Correção em partida `IN_PROGRESS`: **assumido** que segue a mesma regra dos demais adapters — só o monitor responsável (`match.monitor_id`) — por ainda não ter sido confirmado explicitamente com o time; sinalizado como pendência de validação.
    4. WebSocket/Push seguem fora de escopo (mesmo débito técnico do UC016, não resolvido parcialmente aqui).
  - `core/business/match/{undo_last_event,delete_event}_port.py` + `business/match/{undo_last_event,delete_event}_adapter.py`: dois casos de uso/endpoints com contratos diferentes (um localiza o alvo sozinho, o outro recebe `event_id`), mas 100% da regra de negócio compartilhada via `business/match/_correction_shared.py` — validação de partida corrigível (`validate_match_correctable`, aceita `IN_PROGRESS` ou `FINISHED`), bloqueio de tipos não corrigíveis (RN18: `MATCH_STARTED`/`MATCH_END`/`PERIOD_START`/`PERIOD_END`), reversão de expulsão (RN20-23: deletar `EXPULSION` diretamente, ou o 2º `CARD_YELLOW`/`CARD_RED` que a gerou, correlacionados por `player_id`+`clock_seconds`), recálculo de placar por recontagem de eventos restantes em vez de decremento manual (RN25-27, com a fronteira do set em andamento no último `SET_END` ainda vivo — sets já fechados não são recalculados a partir de eventos), recálculo de sets de vôlei com soft delete da linha de `match_sets` (RN28 / ADR 0002 "Momento 5"), recálculo de pênaltis (Fluxo Alternativo 8) e o alerta crítico de correção pós-jogo (RN29-35, Fluxo Alternativo 7) — sem nunca alterar `winner_id` automaticamente (RN31), só sinalizar para o monitor decidir manualmente.
  - Novos valores em `AuditAction`: `MATCH_EVENT_UNDONE`, `MATCH_EVENT_DELETED` (correção em partida `IN_PROGRESS`) e `MATCH_POST_FINISH_CORRECTION` (qualquer correção em partida `FINISHED`, com destaque especial, independentemente de o placar ter mudado).
  - `web/controllers/match_controller.py` ampliado com `POST /api/match/{match_id}/event/undo` e `DELETE /api/match/{match_id}/event/{event_id}`, atrás de `require_monitor`; `MatchManagementResponse` ganhou os campos opcionais `reactivated_player_id` (preenchido quando a correção reverteu uma expulsão) e `correction_alert` (preenchido só quando o placar de uma partida `FINISHED` muda).
  - Migration `a1c017f00d17_v12`: substitui a `UniqueConstraint` de `match_sets` por índice único parcial e reescreve `prevent_match_sets_mutation()`, encadeada no head da árvore do Alembic (`3fbd6f2e9a41` → `a1c017f00d17`).
  - Testes unitários novos em `tests/unit/business/match/test_correction_adapters.py` (22 testes): autorização por status (inclusive "qualquer Monitor" em `FINISHED`), bloqueio de tipos não corrigíveis, busca do último evento corrigível pulando eventos estruturais, recálculo de placar ao deletar gol, reversão de expulsão (direta, 2º amarelo, vermelho direto, e o caso em que o 1º amarelo **não** deve mexer na expulsão), correção de `SET_END` com soft delete do `match_set` e recontagem de sets, e o alerta de correção pós-jogo (com e sem mudança de placar, e a ação de auditoria correta em cada status). Suíte completa (`pytest tests/unit/`) passando: 246/246.
  - **Débito técnico assumido:** mesma dependência de WebSocket/SSE e Push Notification da Fase 6 que os demais UCs de Partida; nenhum dos dois endpoints publica em canal algum hoje.
...
7. Fechar o débito técnico introduzido pelo UC014: adicionar chamadas ao `AuditLogger` (e os valores correspondentes em `AuditAction`) nos sete adapters de `business/match/` que hoje não registram auditoria (gol, cartão, pausar/retomar cronômetro, período, set) — ver observação na Fase 5. **Adiado conscientemente uma terceira vez:** nem o UC015 nem o UC017 fecharam esse débito (cada um registra apenas a própria auditoria), para não misturar o débito de um UC com a implementação do outro. Segue pendente, agora como próximo passo imediato (item 10). Também vale atualizar o `Status` do [ADR 0002](../adr/ADR002_NormalizacaoModalidade.md) para refletir que a normalização já foi implementada.
...
9. ~~Fechar a Fase 5 com UC017 (Corrigir Evento) — único caso de uso restante da fase.~~ — concluído: desfazer último evento corrigível e deletar evento específico da timeline (`/event/undo`, `/event/{event_id}`), com reversão de expulsão, recálculo de placar/sets/pênaltis por recontagem, e alerta crítico + auditoria com destaque especial para correções em partida já `FINISHED` (sem alterar `winner_id` automaticamente). Migration `a1c017f00d17_v12` ajustou a trigger de imutabilidade de `match_sets` (Opção A — soft delete, preservando a garantia original do ADR 0002) em vez de removê-la. 22 testes unitários novos (`test_correction_adapters.py`); suíte completa em 246/246. **Fase 5 (Gestão de Partidas) está oficialmente concluída.**
10. Próximo passo imediato: (a) fechar o débito técnico de auditoria do UC014 (item 7, adiado duas vezes) e (b) validar com o time a pendência explícita deixada pelo UC017 (autorização de correção em partida `IN_PROGRESS` — hoje restrita ao monitor responsável original, decisão assumida e não confirmada). Depois disso, seguir para as Fases 6–7 na ordem já prevista, mantendo a Fase 8 (testes/CI/docs) em paralelo contínuo — incluindo a dívida de testes unitários de UC005 a UC010 registrada na Fase 3 e os `GET`s de chaveamento e de partida registrados como débito técnico nas Fases 4 e 5.